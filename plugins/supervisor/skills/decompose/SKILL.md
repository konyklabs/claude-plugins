...
Spawn `supervisor:scout` (several in parallel if the tree is large) for:
...
through /supervisor:consult if this session is on a cheaper model.

## 3. Order and parallelism, computed

Write the slices as JSON (`.supervisor/slices.json`: `id`, `files`, `deps`,
`command`, `dod` per slice) and let the script order them:

```
python3 "${CLAUDE_PLUGIN_ROOT}/bin/supervisor.py" plan build .supervisor/slices.json --name <plan>
```
...
that breaks parallel worktrees. It writes `.supervisor/plan.md` and
`.supervisor/plan.json`; `plan check .supervisor/plan.json` re-validates after an
edit. Slices within a level run in parallel in worktrees: `supervisor.py run-level
.supervisor/plan.json --level N` supervises them (worktree per slice, retry
...
One spec per slice, via /supervisor:delegate's template, saved under
`.supervisor/specs/`. The **Decisions already made** section is where the
decomposition's reasoning goes: the worker must not re-open a cut.

## 5. The plan file

`.supervisor/plan.md` is what `plan build` wrote; add the decisions block
