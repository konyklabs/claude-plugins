---
name: brief
description: Interviews the developer about a task with at most five structured questions and writes the brief the supervisor flow needs — task, checkable definition of done, evidence command, out of scope, decisions, assumptions, procedure — then lints it. Use before any non-trivial task in an expensive-model session, when a prompt is a sentence, or when asked to write a brief, a spec, or a definition of done.
...
the gaps with at most five questions, writes `.supervisor/brief.md` in the one
...
the result to `.supervisor/brief.md`. The slug is kebab-case from the task, at
...
```
python3 "${CLAUDE_PLUGIN_ROOT}/bin/supervisor.py" brief check .supervisor/brief.md
```
...
> Brief saved to .supervisor/brief.md. Type /supervisor:triage to start; the table comes before any work.
