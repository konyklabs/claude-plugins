...
**Expensive tier (this session, or `supervisor:architect` via /supervisor:consult)**
...
**Opus, high (`supervisor:senior-implementer`)**
...
**Sonnet, medium (`supervisor:implementer`)**
...
|---|-------|------|-------|---------------|
| 1 | inventory fixtures under tests/ | haiku | supervisor:scout | look-up |
| 2 | decide fixture ownership (conftest vs plugin) | expensive | this session | ambiguous, cross-cutting |
| 3 | port tests/api/* to the new fixture | sonnet | supervisor:implementer | mechanical, spec-able |
| 4 | make the savepoint fixture work under xdist | opus | supervisor:senior-implementer | intricate, must stay green |
```

Then, for each row that is not "this session", go to /supervisor:delegate. For a
