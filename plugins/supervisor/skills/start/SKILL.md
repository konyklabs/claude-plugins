...
memory), `Write` it to `.supervisor/brief.md`, then:
...
cuts by coupling, `.supervisor/slices.json`, then
...
- **explore instead**: the task was not a task yet. Run `mode explore`, leave
  the brief on disk as a draft, and end with: `Run /supervisor:explore <the
  question>`.

## 9. Hand off

When the turn ends with a go, the record is `.supervisor/brief.md`, the tier
