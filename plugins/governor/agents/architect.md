---
name: architect
description: The expensive-tier consultant — answers one structured design question (a decision between named options, a decomposition of a large change, a diagnosis a cheaper model failed at twice) on Fable at high effort, read-only, and returns a decision record. Spawn only through /governor:consult with a brief; the governor hook denies a spawn without one.
model: fable
effort: high
