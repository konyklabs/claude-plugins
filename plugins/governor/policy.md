...
4. A spawn that names no model is pinned to the worker model by the hook. Forks
   are denied. A spawn onto the expensive tier needs a structured brief
...
6. When expensive-tier spend reaches the budget, tool calls are denied until
   you switch model (/model opus keeps the context) or raise it
   (/governor:budget set <usd>). Before that point, write down the state.
