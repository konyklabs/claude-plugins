...
- `area`: kebab-case, one word where possible (`auth`, `org`, `credentials`, `docs`).
...
and `playwright-run.json`): `npx playwright test --list --reporter=json`
...
{"tiled_at": "2026-09-04T18:30:00Z",
 "tiles": [
  {"id": "auth.sign-in.valid-password", "area": "auth", "kind": "rule", "flow": "auth.sign-in",
   ...
   "tests": ["e2e/auth.spec.ts::signs in with a valid password"], "cases": ["TC-auth-001"],
   "status": "covered"}
 ],
