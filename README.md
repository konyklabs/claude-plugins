...
| `governor.py run-level PLAN.json --level N [--parallel K] [--retries N]` / `runs [PLAN]` | every slice of a level as a supervised headless worker: worktree each under `.governor/wt/<plan>/<id>`, retry on overload, resumable index, one `VERDICT:` line per slice |
